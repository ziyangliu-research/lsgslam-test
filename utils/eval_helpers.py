...
def align(model, data, align_traj=True):
...
def evaluate_ate(gt_traj, est_traj, align_traj=True):
    """
    Input : 
    ...
    _, _, trans_error = align(gt_traj_pts, est_traj_pts, align_traj=align_traj)
...
# 训练过程中渲染某张图片
def plot_progress(params, data, time_idx, now_frame, sil_thres, plot_dir=None, iter=-1, loss=None):
    # Get current frame Gaussians
    ...
    if loss is None:
        fig_title = f"Rendering frame: {time_idx} | Now Frame: {now_frame} | Iter: {iter}"
    else:
        fig_title = f"Rendering frame: {time_idx} | Now Frame: {now_frame} | Iter: {iter} | Loss: {loss}"
    plot_rgbd_silhouette(data['im'], data['depth'], im, rastered_depth, presence_sil_mask, diff_depth_l1,
                            psnr, depth_l1, fig_title, plot_dir=plot_dir, plot_name=str(time_idx)+'_in_'+str(now_frame)+'_withIter_'+str(iter), save_plot=True,
                            wandb_run=None, wandb_step=None, 
                            wandb_title=None)
...
def eval_online(dataset, all_params, num_frames, eval_online_dir, sil_thres,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color, depth, intrinsics, pose, depth_original, global_feature = dataset[time_idx]
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
         mapping_iters, add_new_gaussians, wandb_run=None, wandb_save_qual=False, eval_every=1, save_frames=False, align_traj=True):
    ...
    try:
        ...
        gt_w2c_list = valid_gt_w2c_list
        # Calculate ATE RMSE
        ate_rmse = evaluate_ate(gt_w2c_list, latest_est_w2c_list, align_traj=align_traj)
        print("Final Average ATE RMSE: {:.2f} cm".format(ate_rmse*100))
        ate_rmse = evaluate_ate(gt_w2c_list, latest_est_w2c_list, align_traj=False)
        print("Final Average ATE RMSE: {:.2f} cm".format(ate_rmse*100))
        if wandb_run is not None:
