...
# Explicit sequence selection. If no args are given, run the full final8 set.
if [ "$#" -gt 0 ]; then
    SEQS=("$@")
else
...
for seq in "${SEQS[@]}"; do
    ...
    echo "Timing: online Stage-1 + offline loop/PGO/deformation/SR; final metric eval excluded"
    ...
    # Stage 1: official 200-frame split submaps.
    start=0
    while [ "$start" -lt "$last_idx" ]; do
        ...
        if [ -f "$out/params.npz" ] && [ -f "$out/benchmark_summary_split.json" ]; then
            ...
            set +e
            python -u scripts/tartanair_split_splatam.py configs/tartanair/lsgslam_full_split_8_2.py 2>&1 | tee "$out/run.log"
            status=${PIPESTATUS[0]}
            set -e
            if [ "$status" -ne 0 ]; then echo "FAILED Stage 1: $run_name (exit=$status)"; exit "$status"; fi
        fi
        start="$end"
    done

    # Stage 2: loop detection + loop constraint generation.
    ...
    if [ -f "$loop_marker" ] && [ -f "$loop_timing" ]; then
        ...
        set +e
        python -u scripts/tartanair_loop_closure.py configs/tartanair/lsgslam_full_split_8_2.py 2>&1 | tee "$seq_root/loop_closure.log"
        status=${PIPESTATUS[0]}
        set -e
        stage2_end_ns="$(date +%s%N)"
        if [ "$status" -ne 0 ]; then echo "FAILED Stage 2: $seq (exit=$status)"; exit "$status"; fi
        write_wall_timing_json "$stage2_start_ns" "$stage2_end_ns" "$loop_timing" "released loop detection + loop constraint generation wall time"
        touch "$loop_marker"
    fi

    # Stage 3: fixed timing launcher; clean only incomplete Stage-3 outputs.
    ...
    if [ -f "$backend_marker" ] && [ -f "$backend_summary" ] && [ -f "$backend_timing" ]; then
        ...
        set +e
        python -u tools/loop_closure/tartanair_pose_graph_part_optim_final.py 2>&1 | tee "$seq_root/full_backend.log"
        status=${PIPESTATUS[0]}
        set -e
        if [ "$status" -ne 0 ]; then echo "FAILED Stage 3: $seq (exit=$status)"; exit "$status"; fi
        if [ ! -f "$backend_timing" ]; then echo "FAILED Stage 3 timing: $backend_timing was not produced"; exit 1; fi
...
echo
echo "================================================================================"
echo "Selected sequences completed. Running unified PSNR/SSIM/LPIPS evaluation..."
echo "================================================================================"
PYTHONPATH="$ROOT_DIR" python tools/evaluate_tartanair_full_unified.py "${SEQS[@]}" --root "$FULL_ROOT" --data-root "$DATA_ROOT"

echo
echo "=============================== FINAL TABLE ==============================="
PYTHONPATH="$ROOT_DIR" python tools/summarize_tartanair_full_split_benchmarks.py "${SEQS[@]}" --root "$FULL_ROOT"
