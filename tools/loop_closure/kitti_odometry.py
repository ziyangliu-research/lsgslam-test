...
class KittiEvalOdom():
    ...
    def eval_kitti(self, gt_poses_dict, est_poses_dict, result_dir, 
                    alignment=None,
                    seq='0'):
            """Evaulate required/available sequences
            ...
            """
            # seq_list = ["{:02}".format(i) for i in range(0, 11)]
            gt_poses = copy.deepcopy(gt_poses_dict)
            est_poses = copy.deepcopy(est_poses_dict)
            ...
            # Pose alignment to first frame
            idx_0 = 0
            pred_0 = est_poses[idx_0]
            gt_0 = gt_poses[idx_0]
            for cnt in est_poses:
                est_poses[cnt] = np.linalg.inv(pred_0) @ est_poses[cnt]
                gt_poses[cnt] = np.linalg.inv(gt_0) @ gt_poses[cnt]

            if alignment == "scale":
                est_poses = self.scale_optimization(gt_poses, est_poses)
            elif alignment == "scale_7dof" or alignment == "7dof" or alignment == "6dof":
                # get XYZ
                xyz_gt = []
                xyz_result = []
                for cnt in est_poses:
                    ...
                    xyz_result.append([est_poses[cnt][0, 3], est_poses[cnt][1, 3], est_poses[cnt][2, 3]])
                xyz_gt = np.asarray(xyz_gt).transpose(1, 0)
                xyz_result = np.asarray(xyz_result).transpose(1, 0)
                ...
                for cnt in est_poses:
                    est_poses[cnt][:3, 3] *= scale
                    if alignment=="7dof" or alignment=="6dof":
                        est_poses[cnt] = align_transformation @ est_poses[cnt]
            
            # compute sequence length
            seq_len = self.compute_trajectory_length(gt_poses)
            avg_seq_lens.append(seq_len)

            # compute sequence errors
            seq_err = self.calc_sequence_errors(gt_poses, est_poses)
            ...
            # Compute ATE
            ate = self.compute_ATE(gt_poses, est_poses)
            seq_ate.append(ate)
            print("ATE (m): ", ate)

            # Compute RPE
            rpe_errors = self.compute_RPE(gt_poses, est_poses)
            rpe_trans = np.mean(np.asarray(rpe_errors['trans']))
            rpe_rot = np.mean(np.asarray(rpe_errors['rot']))
            self.save_RPE_errors(rpe_errors, error_dir + "/" + "RPE_errors.txt")
            seq_rpe_trans.append(rpe_trans)
            seq_rpe_rot.append(rpe_rot)
            print("RPE (m): ", rpe_trans)
            print("RPE (deg): ", rpe_rot * 180 /np.pi)
            ...
            # Plotting
            self.plot_trajectory(gt_poses, est_poses, seq)
            self.plot_error(avg_segment_errs, seq)

            # Save result summary
            self.write_result(f, seq, [ave_t_err, ave_r_err, ate, rpe_trans, rpe_rot])
