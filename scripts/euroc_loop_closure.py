#!/usr/bin/env python3
"""EuRoC loop-closure entry point with benchmark-clean timing.

Uses the released LSG-SLAM loop-closure logic and native EurocDataset routing.
The read-only final eval() at the end of each temporary loop-pair rgbd_slam run
is skipped because it changes neither the loop constraint nor saved parameters
and would otherwise contaminate offline algorithm time.
"""
...
def main():
    ...
    parser.add_argument("experiment", type=str, help="Path to EuRoC experiment config")
