...
def demo(args):
    ...
    with torch.no_grad():
        left_images = sorted(os.listdir(args.left_imgs), key=lambda x:float(x[:-4]))
        right_images = sorted(os.listdir(args.right_imgs), key=lambda x:float(x[:-4]))
        print(f"Found {len(left_images)} images. Saving files to {output_directory}/")

        left_images = left_images[::1]
        right_images = right_images[::1]
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            image1 = load_image(os.path.join(args.left_imgs, imfile1))
            image2 = load_image(os.path.join(args.right_imgs, imfile2))
            ...
            filename = os.path.join(output_directory, imfile1)
            plt.imsave(filename, disp.squeeze(), cmap='jet')
            if args.save_numpy:
                np.save(output_directory / f"{imfile1[:-4]}.npy", disp.squeeze())
