...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # print(config_dict["camera_params"])
        self.depth_filter_near = config_dict["camera_params"]["depth_filter_near"]
        self.depth_filter_far = config_dict["camera_params"]["depth_filter_far"]
        ...
        self.color_paths, self.depth_paths, self.embedding_paths, self.feature_paths = self.get_filepaths()
        ...
        if len(self.feature_paths) > 0 and len(self.color_paths) != len(self.feature_paths):
            raise ValueError("Number of color images and global features must be the same.")
        ...
        if len(self.feature_paths) > 0:
            self.feature_paths = self.feature_paths[self.start : self.end : stride]
    ...
    def __getitem__(self, index):
        ...
        if ".png" in depth_path:
            ...
            depth_original = np.load(depth_path)
            mask1 = depth > self.depth_filter_near
            mask2 = depth < self.depth_filter_far
            mask = mask1 & mask2
            depth[~mask] = 0.0
            # depth_height = depth.shape[0]
            # depth_width = depth.shape[1]
            # depth[:int(depth_height / 3), ...] = 0
            # depth_original[:int(depth_height / 3), ...] = 0
            # # depth[:, int(depth_width / 2):, ...] = 0
        ...
        if depth_original is not None:
            depth_original = self._preprocess_depth(depth_original)
            depth_original = torch.from_numpy(depth_original)
        ...
        return (
            ...
            intrinsics.to(self.device).type(self.dtype),
            pose.to(self.device).type(self.dtype),
            depth_original.to(self.device).type(self.dtype),
            global_feature,
            # self.retained_inds[index].item(),
        )