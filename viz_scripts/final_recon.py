...
def load_scene_data(scene_path, first_frame_w2c, intrinsics):
    ...
    return rendervar, depth_rendervar, all_w2cs, all_gt_w2cs
...
def render(w2c, k, timestep_data, timestep_depth_data, cfg):
    with torch.no_grad():
        cam = setup_camera(cfg['viz_w'], cfg['viz_h'], k, w2c, cfg['viz_near'], cfg['viz_far'], depth_threshold=105.50137862302245)
        white_bg_cam = Camera(
            ...
            depth_threshold=105.50137862302245,
...
def visualize(scene_path, cfg):
    ...
    scene_data, scene_depth_data, all_w2cs, all_gt_w2cs = load_scene_data(scene_path, w2c, k)
    ...
    if cfg['visualize_cams']:
        ...
        gt_cam_centers = []
        gt_cam_colormap = plt.get_cmap('winter')
        for i_t in range(num_t):
            frustum = o3d.geometry.LineSet.create_camera_visualization(w, h, k, all_gt_w2cs[i_t], frustum_size)
            frustum.paint_uniform_color(np.array(gt_cam_colormap(i_t * norm_factor / num_t)[:3]))
            vis.add_geometry(frustum)
            gt_cam_centers.append(np.linalg.inv(all_gt_w2cs[i_t])[:3, 3])
        
        gt_cols = []
        gt_line_colormap = plt.get_cmap('winter')
        for line_t in range(total_num_lines):
            gt_cols.append(np.array(gt_line_colormap((line_t * norm_factor / total_num_lines)+norm_factor)[:3]))
        gt_cols = np.array(gt_cols)
        all_gt_cols = [gt_cols]
        gt_out_pts = [np.array(gt_cam_centers)]
        linesets = make_lineset(gt_out_pts, all_gt_cols, num_lines)
        lines = o3d.geometry.LineSet()
        lines.points = linesets[0].points
        lines.colors = linesets[0].colors
        lines.lines = linesets[0].lines
